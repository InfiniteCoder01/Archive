...
public class Parser {
    ...
    private Statement blockStatement(boolean unclosing) {
        ...
        while (!match(TokenType.OPERATOR, "}")) {
            ...
            if (match(TokenType.EOF)) {
                if (!unclosing) {
                    throw new RuntimeException("Finded EOF, expected }!");
                }
                break;
            }
        }
    ...
    private Value value() {
        ...
        throw new RuntimeException("Unknown value: " + peek(0).getValue());
    }

    private boolean match(TokenType type) {
        if (peek(0).getType() != type) return false;
        index++;
        return true;
    }

    private boolean match(TokenType type, String val) {
        return match(new Token(type, val, peek(0).getLine()));
    }
    ...
    private boolean lookMatch(int relPos, TokenType type, String val) {
        return peek(relPos).equals(new Token(type, val, peek(relPos).getLine()));
    }

    private Token consume(TokenType type, String val) {
        if (peek(0).equals(new Token(type, val, peek(0).getLine()))) {
            index++;
            return peek(-1);
        }
        errors.add(new ParseError(peek(0).getLine(), new RuntimeException("Error finded " + peek(0).getValue() + ", expected " + val)));
        return null;
    }
    ...
    private Token consume(TokenType type) {
        ...
        errors.add(new ParseError(peek(0).getLine(), new RuntimeException("Error finded " + peek(0).getType() + ", expected " + type)));
        return null;
    }

    private boolean match(Token token) {
        if (!peek(0).equals(token)) return false;
        index++;
        return true;
    }
